...
from .. import hookimpl, CondaBeforeAction, CondaSessionClass
...
def cache_session(session: type[BasicAuthSession]):
    ...
    def wrapper(channel_name: str) -> BasicAuthSession:
        ...
        if _SESSION_OBJECT_CACHE.cache.get(channel_name) is None:
            _SESSION_OBJECT_CACHE.cache[channel_name] = BasicAuthSession(channel_name=channel_name)
...
def conda_session_classes():
    ...
    """
    yield CondaSessionClass(name=PLUGIN_NAME, session_class=BasicAuthSession)

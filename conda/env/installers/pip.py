...
from ..pip_util import pip_subprocess, get_pip_installed_packages
...
def _pip_install_via_requirements(prefix, specs, args, *_, **kwargs):
    ...
    try:
        ...
        requirements = Utf8NamedTemporaryFile(
            mode="w", prefix="condaenv.", suffix=".requirements.txt", dir=pip_workdir, delete=False
        )
        requirements.write("\n".join(specs))
        requirements.close()
        # pip command line...
...
def install(*args, **kwargs):
    with Spinner(
        "Installing pip dependencies", not context.verbosity and not context.quiet, context.json
    ):

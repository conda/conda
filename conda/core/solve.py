...
class Solver(object):
    ...
    def solve_final_state(self, prune=NULL, force_reinstall=NULL, deps_modifier=None,
        ...
        if specs_to_remove:
            if force_remove:
                solution = tuple(dist for dist in solution
                                 if not any(spec.match(dist) for spec in specs_to_remove))
                solution = IndexedSet(r.dependency_sort({d.name: d for d in solution}))
                return IndexedSet(index[d] for d in solution)

            # If the specs we're removing involve features, then we need to invoke the SAT
        ...
        # keep packages matching specs from history that are causing inconsistencies
        ...
        if inconsistent_dists:
            for dist in inconsistent_dists:
                ...
                # pop and save matching dist from solution
        ...
        # only if spec not in specs_to_add

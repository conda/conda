...
class Solver(object):
    ...
    def solve_for_diff(
        ...
        final_precs = self.solve_final_state(
            update_modifier, deps_modifier, prune, ignore_pinned, force_remove, should_retry_solve
        )
    ...
    def solve_final_state(
        ...
        with Spinner(
            ...
            if ssc.add_back_map:
                orphan_precs = (
                    set(solution_precs) - set(ssc.solution_precs) - set(ssc.add_back_map)
                )
    ...
    def _remove_specs(self, ssc):
        if self.specs_to_remove:
            ...
            feature_names = set(
                concat(spec.get_raw_value("track_features") for spec in _track_fts_specs)
            )
            ...
            if matches_for_spec:
                ...
                if target_prec.is_unmanageable:
                    ...
                    ssc.specs_map[pkg_name] = MatchSpec(
                        ssc.specs_from_history_map[pkg_name], target=target_prec.dist_str()
                    )
                else:
    ...
    def _run_sat(self, ssc):
        ...
        conflicting_specs = set(
            ssc.r.get_conflicting_specs(tuple(final_environment_specs), self.specs_to_add) or []
        )
        while conflicting_specs:
            ...
            if log.isEnabledFor(DEBUG):
                log.debug(
                    "conflicting specs: %s",
                    dashlist(s.target if s.target else s for s in conflicting_specs),
                )
            ...
            conflicting_pinned_specs = groupby(
                lambda s: MatchSpec(s, optional=True) in ssc.pinned_specs, conflicting_specs
            )
            ...
            if conflicting_pinned_specs.get(True):
                ...
                in_specs_map_or_specs_to_add = (set(in_specs_map) | set(self.specs_to_add)) - set(
                    pinned_conflicts
                )
            ...
            if specs_modified:
                conflicting_specs = set(
                    ssc.r.get_conflicting_specs(tuple(final_environment_specs), self.specs_to_add)
                )
            else:
        ...
        if current_conda_prefix_rec:
            ...
            conda_newer_precs = sorted(
                SubdirData.query_all(
                    conda_newer_spec, self.channels, self.subdirs, repodata_fn=self._repodata_fn
                ),
            ...
            if conda_newer_precs:
                ...
                """
                    ...
                    % (CONDA_VERSION, latest_version, add_channel),
                    file=sys.stderr,
                )
    ...
    def _prepare(self, prepared_specs):
        ...
        if hasattr(self, "_index") and self._index:
            ...
            reduced_index = get_reduced_index(
                self.prefix, self.channels, self.subdirs, prepared_specs, self._repodata_fn
            )
...
def get_pinned_specs(prefix):
    ...
    if exists(pinfile):
        with open(pinfile) as f:
            from_file = (
                i for i in f.read().strip().splitlines() if i and not i.strip().startswith("#")
            )
    else:
        from_file = ()

    return tuple(MatchSpec(s, optional=True) for s in concatv(context.pinned_packages, from_file))

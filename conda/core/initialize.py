...
def _config_xonsh_content(conda_prefix):
    conda_exe = join(conda_prefix, BIN_DIRECTORY, "conda.exe" if on_win else "conda")
    if on_win:
        # xonsh accepts either separator on Windows; converting only the
        # backslashes (instead of using win_path_to_unix's MSYS-style
        # `/c/...`) keeps the path Windows-native and avoids the cygpath
        # dependency on bare installations.
...
def _config_xonsh_content_to_add_condabin_to_path(conda_prefix):
    ...
    if on_win:
        # xonsh accepts either separator on Windows; see _config_xonsh_content
        # for why we prefer backslash_to_forwardslash over win_path_to_unix.
        from ..activate import backslash_to_forwardslash

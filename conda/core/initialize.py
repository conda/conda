...
def make_entry_point(target_path, conda_prefix, module, func):
    ...
    if on_win:
        ...
        new_ep_content = generate_shebang_for_entry_point(python_path, ps1_workaround=True)
...
def _config_fish_content(conda_prefix):
    ...
    if test -f %(python_exe)s
        eval %(python_exe)s "-I" "-m" "conda" "shell.fish" "hook" $argv | source
    end
    # <<< conda initialize <<<
    """) % {
...
def _config_xonsh_content(conda_prefix):
    ...
    if !(test -f "{python_exe}"):
        ...
        _mod = _ModuleType("xontrib.conda",
                        "Autogenerated from $({python_exe} -I -m conda shell.xonsh hook)")
        __xonsh__.execer.exec($("{python_exe}" "-I" "-m" "conda" "shell.xonsh" "hook"),
                            glbs=_mod.__dict__,
                            filename="$({python_exe} -I -m conda shell.xonsh hook)")

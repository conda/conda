...
from ..common.compat import ensure_binary, iteritems, string_types
...
def subprocess_call(command, env=None, path=None, stdin=None, raise_on_error=True):
    ...
    env = {encode_for_env_var(k): encode_for_env_var(v)
           for k, v in iteritems(os.environ if env is None else env)}
    ...
    p = Popen([encode_for_env_var(c) for c in command], cwd=cwd, stdin=PIPE,

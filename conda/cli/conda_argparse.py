...
def _configure_builtin_parser(sub_parsers, name: str) -> ArgumentParser:
    for builtin, module_path, _help_text, extra_kwargs in _BUILTIN_SUBCOMMANDS:
        if builtin == name:
            mod = import_module(module_path)
            return mod.configure_parser(sub_parsers, **extra_kwargs)
    raise KeyError(name)


def _preview_plugin_commands() -> set[str]:
...
def generate_parser(**kwargs) -> ArgumentParser:
    ...
    preview_plugin_commands = _preview_plugin_commands()
    for name, module_path, help_text, extra_kwargs in _BUILTIN_SUBCOMMANDS:
        if name not in preview_plugin_commands:
...
def configure_parser_plugins(sub_parsers) -> None:
    ...
    for name, plugin_subcommand in plugin_subcommands.items():
        ...
        if preview and name in BUILTIN_COMMANDS:
            parser = _configure_builtin_parser(sub_parsers, name)
        else:

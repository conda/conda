...
@FOR /F "delims=" %%i IN ('@CALL %CONDA_EXE% shell.cmd.exe activate %*') DO @SET "_TEMP_SCRIPT_PATH=%%i"
...
:DO_DEACTIVATE
@FOR /F "delims=" %%i IN ('@CALL %CONDA_EXE% shell.cmd.exe deactivate %*') DO @SET "_TEMP_SCRIPT_PATH=%%i"
@IF "%_TEMP_SCRIPT_PATH%"=="" GOTO :ErrorEnd
...
:DO_REACTIVATE
@FOR /F "delims=" %%i IN ('@CALL %CONDA_EXE% shell.cmd.exe reactivate') DO @SET "_TEMP_SCRIPT_PATH=%%i"
@IF "%_TEMP_SCRIPT_PATH%"=="" GOTO :ErrorEnd
@CALL "%_TEMP_SCRIPT_PATH%"
@DEL /F /Q "%_TEMP_SCRIPT_PATH%"
@SET _TEMP_SCRIPT_PATH=

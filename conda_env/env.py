...
def from_environment(name, prefix, no_builds=False, ignore_channels=False):
    ...
    """
    installed = linked_data(prefix, ignore_channels=ignore_channels)
    conda_names = set(p['name'] for p in installed.values())
    ...
    def _fmt(p):
        ...
        return '='.join((name, p['version'], p['build']))

    dependencies = list(map(_fmt, sorted(installed.values(), key=lambda a: a['name'])))
    if len(pip_pkgs) > 0:

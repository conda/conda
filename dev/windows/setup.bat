...
CALL conda create -n conda-test-env -y --solver=libmamba python=%PYTHON% pywin32 --file=tests\requirements.txt || goto :error
...
CALL conda install -y -c "conda-forge::menuinst>=2" || goto :error

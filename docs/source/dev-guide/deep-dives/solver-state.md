...
This document is a technical reference for how solver inputs are assembled, which
might not be the best way to learn about how the solver works. For that, refer to
{doc}`install` and {doc}`solvers`.
:::

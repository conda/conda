...
.. note::
  ...
  In the Fish shell, the command prompt is not constructed from an
  environment variable but is generated by shell functions. These
  prompt functions are redefined by ``conda init`` regardless of the
  value of ``changeps1``. The redefinition of the Fish prompt
  functions can be disabled by setting the environment variable
  ``CONDA_DISABLE_FISH_PROMPT`` to any value.

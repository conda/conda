...
def test_activate_environment_not_found(tmp_path: Path) -> None:
...
def test_PS1(tmp_path: Path) -> None:
...
def test_PS1_no_changeps1(monkeypatch: MonkeyPatch, tmp_path: Path) -> None:
...
def test_remove_prefix_from_path_1() -> None:
    activator = PosixActivator()
...
def test_remove_prefix_from_path_2() -> None:
...
def test_replace_prefix_in_path_1() -> None:
    activator = PosixActivator()
    original_path = tuple(activator._get_starting_path_list())
...
@pytest.mark.skipif(not on_win, reason="windows-specific test")
def test_replace_prefix_in_path_2(monkeypatch: MonkeyPatch) -> None:
...
def test_build_activate_shlvl_0(env_activate: tuple[str, str, str]) -> None:
    prefix, activate_sh, _ = env_activate
...
def test_get_env_vars_big_whitespace(tmp_env: TmpEnvFixture) -> None:
    with tmp_env() as prefix:
        write_state_file(prefix)
...
def test_get_env_vars_empty_file(tmp_env: TmpEnvFixture) -> None:
    with tmp_env() as prefix:
...
def test_activate_and_deactivate_for_uninitialized_env(conda_cli) -> None:
...
def test_force_uppercase(
    monkeypatch: MonkeyPatch, force_uppercase_boolean: bool
) -> None:
...
class PrePostCommandPlugin:
    ...
    @plugins.hookimpl
    def conda_pre_commands(self) -> Iterator[CondaPreCommand]:
    ...
    @plugins.hookimpl
    def conda_post_commands(self) -> Iterator[CondaPostCommand]:
...
def test_activator_invalid_command_arguments(
    command_args: list[str], expected_error_message: str
) -> None:
    """

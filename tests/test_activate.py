...
class ActivatorUnitTests(TestCase):
    ...
    def test_build_activate_shlvl_0(self):
        with tempdir() as td:
            ...
            _, activate_d_1 = _make_activate_d(td, 'see-me.sh')
            _, activate_d_2 = _make_activate_d(td, 'dont-see-me.bat')
            ...
            with env_var('CONDA_SHLVL', '0'):
                with env_var('CONDA_PREFIX', ''):
                    ...
                    activate_scripts = (activator.path_conversion(activate_d_1),)
                    deactivate_scripts = ()

                    assert builder['set_vars'] == set_vars
                    assert builder['export_vars'] == export_vars
                    assert builder['activate_scripts'] == activate_scripts
                    assert builder['deactivate_scripts'] == deactivate_scripts
    ...
    def test_build_activate_shlvl_2(self):
        with tempdir() as td:
            ...
            _, deactivate_d_1 = _make_deactivate_d(old_prefix, 'see-me.sh')
            _, deactivate_d_2 = _make_deactivate_d(old_prefix, 'dont-see-me.bat')
            ...
            with env_var('CONDA_SHLVL', '2'):
                with env_var('CONDA_PREFIX', old_prefix):
                    ...
                    export_vars = {
                        ...
                        'CONDA_DEFAULT_ENV': td,
                        'CONDA_PROMPT_MODIFIER': conda_prompt_modifier,
                        'CONDA_POST': "%s:%s" % (old_prefix, td),
                    }
                    activate_scripts = (activator.path_conversion(activate_d_1),)
                    deactivate_scripts = (activator.path_conversion(deactivate_d_1),)
    ...
    def test_activate_same_environment(self):
        with tempdir() as td:
            ...
            _, deactivate_d_1 = _make_deactivate_d(td, 'see-me.sh')
            _, deactivate_d_2 = _make_deactivate_d(td, 'dont-see-me.bat')
            ...
            with env_var('CONDA_SHLVL', '1'):
                with env_var('CONDA_PREFIX', td):
                    ...
                    # new_path_parts = activator._replace_prefix_in_path(td, td)
                    conda_prompt_modifier = "(%s) " % td
                    ps1 = conda_prompt_modifier + os.environ.get('PS1', '')
    ...
    def test_build_deactivate_shlvl_2(self):
        with tempdir() as td:
            ...
            old_prefix = join(td, 'old')
            _, activate_d_1 = _make_activate_d(old_prefix, 'see-me.sh')
            _, activate_d_2 = _make_activate_d(old_prefix, 'dont-see-me.bat')
...
def _make_d(td, extension):
    activate_dir, activate_ignore = _make_activate_d(td, 'ignore.txt')
    deactivate_dir, deactivate_ignore = _make_deactivate_d(td, 'ignore.txt')

    _, activate_script = _make_activate_d(td, 'activate1' + extension)
    _, deactivate_script = _make_deactivate_d(td, 'deactivate1' + extension)

    return {
        "activate_dir": activate_dir,
        "activate_ignore": activate_ignore,
        "activate_script": activate_script,
        "deactivate_dir": deactivate_dir,
        "deactivate_ignore": deactivate_ignore,
        "deactivate_script": deactivate_script,
    }


def _make_conda_meta(td):
    directory = mkdir_p(join(td, 'conda-meta'))
...
def _make_activate_d(td, script):
    directory = mkdir_p(join(td, 'etc', 'conda', 'activate.d'))

    script = join(directory, script)
    touch(script)

    return (directory, script)


def _make_deactivate_d(td, script):
    directory = mkdir_p(join(td, 'etc', 'conda', 'deactivate.d'))
...
class ShellWrapperUnitTests(TestCase):
    ...
    def setUp(self):
        ...
        prefix_dirname = uuid4().hex[:4] + ' ' + uuid4().hex[:4]
        self.prefix = join(td, prefix_dirname)

        _make_conda_meta(self.prefix)
    ...
    def test_posix_basic(self):
        activator = PosixActivator()
        scripts = _make_d(self.prefix, activator.script_extension)

        with captured() as c:
            assert activate_main(('', 'shell.posix', 'activate', self.prefix)) == 0
        assert not c.stderr
        ...
        with env_vars({
            ...
            activator = PosixActivator()
            with captured() as c:
                assert activate_main(('', 'shell.posix', 'reactivate')) == 0
            assert not c.stderr
            ...
            with env_vars({
                'CONDA_POST': '%s:%s' % (self.prefix, self.prefix),
            }):
                post = PosixActivator()
                with captured() as c:
                    assert activate_main(('', 'shell.posix', 'post')) == 0
                assert not c.stderr
                post_data = c.stdout

                path = post._replace_prefix_in_path(self.prefix, self.prefix)
                path = post.pathsep_join(path)
                assert post_data == dals("""
                \\unset CONDA_POST
                \\export PATH='%(path)s'
                """) % {
                    'path': path,
            ...
            with captured() as c:
                assert activate_main(('', 'shell.posix', 'deactivate')) == 0
            assert not c.stderr
            ...
            with env_vars({
                ...
                path = post._remove_prefix_from_path(self.prefix)
                path = post.pathsep_join(path)
                assert post_data == dals("""
                \\unset CONDA_POST
                \\export PATH='%(path)s'
    ...
    def test_cmd_exe_basic(self):
        ...
        with captured() as c:
            assert activate_main(('', 'shell.cmd.exe', 'activate', '', self.prefix)) == 0
        assert not c.stderr
        ...
        with env_vars({
            ...
            with env_vars({
                ...
                post = CmdExeActivator()
                with captured() as c:
                    assert activate_main(('', 'shell.cmd.exe', 'post')) == 0
                assert not c.stderr
                post_result = c.stdout

                with open(post_result) as fh:
                    post_data = fh.read()
                rm_rf(post_result)

                path = activator._replace_prefix_in_path(self.prefix, self.prefix)
                path = post.pathsep_join(path)
                assert post_data == dals("""
                @SET CONDA_POST=
                @SET "PATH=%(path)s"
            ...
            with env_vars({
                ...
                path = activator._remove_prefix_from_path(self.prefix)
                path = post.pathsep_join(path)
                assert post_data == dals("""
                @SET CONDA_POST=
                @SET "PATH=%(path)s"
    ...
    def test_csh_basic(self):
        ...
        with captured() as c:
            assert activate_main(('', 'shell.csh', 'activate', self.prefix)) == 0
        assert not c.stderr
        activate_data = c.stdout
        ...
        with env_vars({
            ...
            activator = CshActivator()
            with captured() as c:
                assert activate_main(('', 'shell.csh', 'reactivate')) == 0
            assert not c.stderr
            reactivate_data = c.stdout
            ...
            with env_vars({
                ...
                post = CshActivator()
                with captured() as c:
                    assert activate_main(('', 'shell.csh', 'post')) == 0
                assert not c.stderr
                post_data = c.stdout
            ...
            with captured() as c:
                assert activate_main(('', 'shell.csh', 'deactivate')) == 0
            assert not c.stderr
            deactivate_data = c.stdout
    ...
    def test_xonsh_basic(self):
        ...
        with captured() as c:
            assert activate_main(('', 'shell.xonsh', 'activate', self.prefix)) == 0
        assert not c.stderr
        activate_result = c.stdout
        ...
        with env_vars({
            ...
            assert reactivate_data == dals("""
            source "%(deactivate1)s"
            $CONDA_POST = '%(native_prefix)s:%(native_prefix)s'
            $CONDA_PROMPT_MODIFIER = '(%(native_prefix)s) '
            $CONDA_SHLVL = '1'
            ...
            with env_vars({
                ...
                post = XonshActivator()
                with captured() as c:
                    assert activate_main(('', 'shell.xonsh', 'post')) == 0
                assert not c.stderr
                post_result = c.stdout
    ...
    def test_fish_basic(self):
        ...
        with captured() as c:
            assert activate_main(('', 'shell.fish', 'activate', self.prefix)) == 0
        assert not c.stderr
        activate_data = c.stdout
        ...
        with env_vars({
            ...
            activator = FishActivator()
            with captured() as c:
                assert activate_main(('', 'shell.fish', 'reactivate')) == 0
            assert not c.stderr
            reactivate_data = c.stdout

            assert reactivate_data == dals("""
            source "%(deactivate1)s";
            set -gx CONDA_POST "%(native_prefix)s:%(native_prefix)s";
            set -gx CONDA_PROMPT_MODIFIER "(%(native_prefix)s) ";
            set -gx CONDA_SHLVL "1";
            ...
            with env_vars({
                ...
                post = FishActivator()
                with captured() as c:
                    assert activate_main(('', 'shell.fish', 'post')) == 0
                assert not c.stderr
                post_data = c.stdout
            ...
            with captured() as c:
                assert activate_main(('', 'shell.fish', 'deactivate')) == 0
            assert not c.stderr
            deactivate_data = c.stdout
    ...
    @pytest.mark.skipif(not on_win, reason="powershell only on Windows")
    def test_powershell_basic(self):
        activator = PowershellActivator()
        scripts = _make_d(self.prefix, activator.script_extension)

        with captured() as c:
            assert activate_main(('', 'shell.powershell', 'activate', self.prefix)) == 0
        assert not c.stderr
        activate_data = c.stdout
        ...
        """) % {
            ...
            'activate1': scripts["activate_script"],
        ...
        with env_vars({
            ...
            activator = PowershellActivator()
            with captured() as c:
                assert activate_main(('', 'shell.powershell', 'reactivate')) == 0
            assert not c.stderr
            reactivate_data = c.stdout

            assert reactivate_data == dals("""
            . "%(deactivate1)s"
            $env:CONDA_POST = "%(prefix)s;%(prefix)s"
            $env:CONDA_PROMPT_MODIFIER = "(%(prefix)s) "
            $env:CONDA_SHLVL = "1"
            ...
            """) % {
                'activate1': scripts["activate_script"],
                'deactivate1': scripts["deactivate_script"],
                'prefix': self.prefix,
            }
            ...
            #     'CONDA_POST': '%s:%s' % (self.prefix, self.prefix),
            # }):
            #     post = PowershellActivator()
            #     with captured() as c:
            #         assert activate_main(('', 'shell.powershell', 'post')) == 0
            #     assert not c.stderr
            #     post_data = c.stdout
            ...
            with captured() as c:
                assert activate_main(('', 'shell.powershell', 'deactivate')) == 0
            assert not c.stderr
            deactivate_data = c.stdout
            ...
            with tempdir() as td:
                with open(join(td, "stdout"), "wt") as stdout:
                    with captured(stdout=stdout) as c:
                        activate_main(('', shell, 'activate', self.prefix))
...
class ShellWrapperIntegrationTests(TestCase):
    ...
    def setUp(self):
        ...
        self.charizard = join(self.prefix, 'envs', 'charizard')

        _make_conda_meta(self.prefix)
        _make_conda_meta(self.charizard)

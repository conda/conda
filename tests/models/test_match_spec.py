...
class MatchSpecTests(TestCase):
    def test_match_1(self):
        ...
        assert MatchSpec("numpy >=1.0.1a py27*").match(
            DPkg("numpy-1.0.1z-py27_1.tar.bz2")
        )
        ...
        assert (
            m2.spec == m3.spec and m2.optional == m3.optional and m2.target != m3.target
        )
        assert m == m4
    ...
    def test_canonical_string_forms(self):
        ...
        assert (
            m(
                "numpy[channel=https://repo.anaconda.com/pkgs/free/osx-64, subdir=linux-64]"
            )
        ...
        assert (
            m("numpy[channel=conda-forge,subdir=noarch]") == "conda-forge/noarch::numpy"
        )
        ...
        assert m("numpy !=1.10=py38_0") == "numpy[version='!=1.10',build=py38_0]"
        assert (
            m("numpy >1.7,!=1.10 py38_0") == "numpy[version='>1.7,!=1.10',build=py38_0]"
        )
        assert m("numpy!=1.10.*") == "numpy!=1.10.*"
    ...
    @pytest.mark.skip(
        reason="key-value features interface has been disabled in conda 4.4"
    )
    ...
    def test_channel_matching(self):
        with env_unmodified(conda_tests_ctxt_mgmt_def_pol):
            ...
            assert (
                ChannelMatch("https://conda.anaconda.org/conda-forge").match(
                    "conda-forge"
                )
                is True
            )
            assert (
                ChannelMatch("conda-forge").match(
    ...
    def test_dist(self):
        with env_unmodified(conda_tests_ctxt_mgmt_def_pol):
            ...
            p = MatchSpec(
                channel="defaults", name="python", version=VersionSpec("3.5*")
            )
            assert p.match(
                Dist(
                    channel="defaults",
...
class TestSpecFromLine(TestCase):
    ...
    def test_comment(self):
        ...
        assert (
            spec_from_line("foo ## comment") == "foo" == self.cb_form("foo ## comment")
        )
    ...
    def test_pip_style(self):
        ...
        assert spec_from_line("foo >=1.0") == "foo >=1.0" == self.cb_form("foo >=1.0")
        assert (
            spec_from_line("FOO-Bar >=1.0")
            == "foo-bar >=1.0"
            == self.cb_form("FOO-Bar >=1.0")
        )
        assert spec_from_line("foo >= 1.0") == "foo >=1.0" == self.cb_form("foo >= 1.0")
        assert spec_from_line("foo > 1.0") == "foo >1.0" == self.cb_form("foo > 1.0")
        assert spec_from_line("foo != 1.0") == "foo !=1.0" == self.cb_form("foo != 1.0")
...
class SpecStrParsingTests(TestCase):
    ...
    def test_dist_str(self):
        for ext in (CONDA_PACKAGE_EXTENSION_V1, CONDA_PACKAGE_EXTENSION_V2):
            m1 = MatchSpec.from_dist_str(
                f"anaconda/{context.subdir}::python-3.6.6-0{ext}"
            )

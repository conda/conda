...
def save_data_source(url, name):
    raw_repodata_str = SubdirData(url).repo_fetch.fetch_latest_parsed()
...
def main():
    ...
    * conda-env has namespace patch
    ...
    conda_env_dicts = tuple(
        info for info in _keep.values() if info["name"] == "conda-env"
    )
    ...
    perl_info_dicts = tuple(
        info for info in _keep.values() if info["name"].startswith("perl-")
    )
    ...
    additional_records = {
        ...
        "sqlite-3.20.1-haaaaaaa_4.tar.bz2": {  # deep cyclical dependency
            "build": "haaaaaaa_4",
            "build_number": 4,
            "depends": ["libedit", "libgcc-ng >=7.2.0", "jinja2 2.9.6"],
    ...
    python_362_records = tuple(
        info
        for info in _keep.values()
    ...
    for info in keep.values():
        for line in info["depends"]:
            ...
            if (
                package_name not in all_package_names
                and package_name not in ignore_names
            ):

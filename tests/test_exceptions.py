...
class ExceptionTests(TestCase):
    def test_TooManyArgumentsError(self):
        ...
        assert (
            ...
            == "<class 'conda.exceptions.TooManyArgumentsError'>"
        )
    ...
    def test_UnknownPackageClobberError(self):
        ...
        colliding_dist_being_linked = "Groot"
        exc = UnknownPackageClobberError(
            target_path, colliding_dist_being_linked, context
        )
        with env_var(
            "CONDA_PATH_CONFLICT",
            "prevent",
    ...
    def test_SharedLinkPathClobberError(self):
        ...
        exc = SharedLinkPathClobberError(
            target_path, incompatible_package_dists, context
        )
        with env_var(
            "CONDA_PATH_CONFLICT",
            "prevent",
    ...
    def test_CondaFileNotFoundError(self):
        ...
        assert not c.stderr
        assert (
            json_obj["exception_type"] == "<class 'conda.exceptions.PathNotFoundError'>"
        )
    ...
    def test_MD5MismatchError(self):
        ...
        assert (
            json_obj["exception_type"]
            == "<class 'conda.exceptions.ChecksumMismatchError'>"
        )
    ...
    def test_PackageNotFoundError(self):
        ...
        assert (
            json_obj["exception_type"]
            == "<class 'conda.exceptions.PackagesNotFoundError'>"
        )
        assert json_obj["message"] == str(exc)
        assert json_obj["error"] == repr(exc)
    ...
    def test_CondaHTTPError(self):
        ...
        with env_var("CONDA_JSON", "yes", stack_callback=conda_tests_ctxt_mgmt_def_pol):
            ...
            assert (
                json_obj["exception_type"]
                == "<class 'conda.exceptions.CondaHTTPError'>"
            )
    ...
    def test_CommandNotFoundError_simple(self):
        ...
        assert (
            json_obj["exception_type"]
            == "<class 'conda.exceptions.CommandNotFoundError'>"
        )
        assert json_obj["message"] == str(exc)
        assert json_obj["error"] == repr(exc)
    ...
    def test_print_unexpected_error_message_upload_2(self, post_mock):
        with env_var(
            "CONDA_JSON", "true", stack_callback=conda_tests_ctxt_mgmt_def_pol
        ):
            with env_var(
                "CONDA_YES", "yes", stack_callback=conda_tests_ctxt_mgmt_def_pol
            ):
                with captured() as c:
                    ExceptionHandler()(_raise_helper, AssertionError())

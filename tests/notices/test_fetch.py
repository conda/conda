...
def test_get_channel_notice_response_timeout_error(
    ...
    with patch("conda.notices.fetch.logger") as mock_logger:
        notices_mock_fetch_session_manager().get.side_effect = requests.exceptions.Timeout
...
def test_get_channel_notice_response_malformed_json(
    ...
    with patch("conda.notices.fetch.logger") as mock_logger:
        add_resp_to_mock(notices_mock_fetch_session_manager, 200, messages, raise_exc=True)

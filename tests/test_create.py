...
def test_install_python2_and_search(clear_pkg_cache):
...
@pytest.mark.integration
def test_run_preserves_arguments(clear_pkg_cache):
...
@pytest.mark.integration
def test_create_install_update_remove_smoketest(clear_pkg_cache):
...
@pytest.mark.integration
def test_install_broken_post_install_keeps_existing_folders(clear_pkg_cache):
...
@pytest.mark.integration
def test_safety_checks(clear_pkg_cache):
...
@pytest.mark.integration
def test_json_create_install_update_remove(clear_pkg_cache):
...
@pytest.mark.integration
def test_not_writable_env_raises_EnvironmentNotWritableError(clear_pkg_cache):
    with make_temp_env() as prefix:
...
@pytest.mark.integration
def test_conda_update_package_not_installed(clear_pkg_cache):
    with make_temp_env() as prefix:
...
@pytest.mark.integration
def test_noarch_python_package_with_entry_points(clear_pkg_cache, noarch_entry_points):
    with make_temp_env("--use-local", "noarch_entry_points") as prefix:
        py_ver = get_python_version_for_prefix(prefix)
        sp_dir = get_python_site_packages_short_path(py_ver)
        py_file = sp_dir + "/noarch_entry_points.py"
        pyc_file = pyc_path(py_file, py_ver).replace("/", os.sep)
        assert isfile(join(prefix, py_file))
        assert isfile(join(prefix, pyc_file))
        exe_path = join(prefix, get_bin_directory_short_path(), "noarch_entry_points")
        if on_win:
            exe_path += ".exe"
        assert isfile(exe_path)
        output = check_output([exe_path, "--help"], text=True)
        assert "usage: noarch_entry_points" in output

        run_command(Commands.REMOVE, prefix, "noarch_entry_points")
...
@pytest.mark.integration
def test_noarch_python_package_without_entry_points(clear_pkg_cache):
...
@pytest.mark.integration
def test_noarch_python_package_reinstall_on_pyver_change(clear_pkg_cache):
    with make_temp_env(
...
@pytest.mark.integration
def test_noarch_generic_package(clear_pkg_cache):
    with make_temp_env("-c", "conda-test", "font-ttf-inconsolata") as prefix:
        assert isfile(join(prefix, "fonts", "Inconsolata-Regular.ttf"))


@pytest.mark.integration
def test_override_channels(clear_pkg_cache):
    with pytest.raises(OperationNotAllowed):
        with env_var(
            "CONDA_OVERRIDE_CHANNELS_ENABLED", "no", stack_callback=conda_tests_ctxt_mgmt_def_pol
        ):
...
@pytest.mark.integration
def test_create_empty_env(clear_pkg_cache):
    with make_temp_env() as prefix:
...
def test_strict_channel_priority(clear_pkg_cache):
    with make_temp_env() as prefix:
...
@pytest.mark.integration
def test_strict_resolve_get_reduced_index(clear_pkg_cache):
...
@pytest.mark.integration
def test_list_with_pip_no_binary(clear_pkg_cache):
    ...
    with make_temp_env("python=" + py_ver, "pip") as prefix:
        ...
        with env_vars(evs, stack_callback=conda_tests_ctxt_mgmt_def_pol):
            ...
            assert any(
                line.endswith("pypi") for line in stdout_lines if line.lower().startswith("flask")
            )
...
@pytest.mark.integration
def test_list_with_pip_wheel(clear_pkg_cache):
    from conda.exports import rm_rf as _rm_rf
...
@pytest.mark.integration
def test_compare_success(clear_pkg_cache):
...
@pytest.mark.integration
def test_compare_fail(clear_pkg_cache):
    with make_temp_env("python=3.6", "flask=1.0.2", "bzip2=1.0.8") as prefix:
        env_file = join(prefix, "env.yml")
        touch(env_file)
        with open(env_file, "w") as f:
...
@pytest.mark.integration
def test_install_tarball_from_local_channel(clear_pkg_cache):
    ...
    with make_temp_env() as prefix, make_temp_channel(["flask-0.12.2"]) as channel:
        ...
        flask_fname = [p for p in PrefixData(prefix).iter_records() if p["name"] == "flask"][0][
            "fn"
        ]
...
@pytest.mark.integration
def test_tarball_install(clear_pkg_cache):
...
@pytest.mark.integration
def test_tarball_install_and_bad_metadata(clear_pkg_cache):
...
def test_update_with_pinned_packages(clear_pkg_cache):
...
@pytest.mark.integration
def test_pinned_override_with_explicit_spec(clear_pkg_cache):
...
@pytest.mark.integration
def test_remove_all(clear_pkg_cache):
...
def test_remove_features(clear_pkg_cache):
...
def test_dash_c_usage_replacing_python(clear_pkg_cache):
...
@pytest.mark.integration
def test_install_prune_flag(clear_pkg_cache):
...
def test_remove_force_remove_flag(clear_pkg_cache):
    with make_temp_env("python") as prefix:
        assert package_is_installed(prefix, "readline")
        assert package_is_installed(prefix, "python")
...
@pytest.mark.integration
def test_install_force_reinstall_flag(clear_pkg_cache):
    with make_temp_env("python") as prefix:
        stdout, stderr, _ = run_command(
            Commands.INSTALL,
            prefix,
...
@pytest.mark.integration
def test_create_no_deps_flag(clear_pkg_cache):
...
@pytest.mark.integration
def test_create_only_deps_flag(clear_pkg_cache):
...
@pytest.mark.integration
def test_install_update_deps_flag(clear_pkg_cache):
...
@pytest.mark.integration
def test_install_only_deps_flag(clear_pkg_cache):
...
@pytest.mark.integration
def test_install_update_deps_only_deps_flags(clear_pkg_cache):
    with make_temp_env("flask=2.0.1", "jinja2=3.0.1") as prefix:
        python = join(prefix, PYTHON_BINARY)
        result_before = subprocess_call_with_clean_env([python, "--version"])
        assert package_is_installed(prefix, "flask=2.0.1")
        assert package_is_installed(prefix, "jinja2=3.0.1")
        run_command(
            Commands.INSTALL, prefix, "flask", "python=3.10", "--update-deps", "--only-deps"
        )
        result_after = subprocess_call_with_clean_env([python, "--version"])
        assert result_before == result_after
        assert package_is_installed(prefix, "flask=2.0.1")
...
def test_install_features(clear_pkg_cache):
...
@pytest.mark.integration
def test_clone_offline_simple(clear_pkg_cache):
    with make_temp_env("bzip2") as prefix:
        assert package_is_installed(prefix, "bzip2")
...
@pytest.mark.integration
def test_conda_config_describe(clear_pkg_cache):
    with make_temp_env() as prefix:
...
@pytest.mark.integration
def test_conda_config_validate(clear_pkg_cache):
    with make_temp_env() as prefix:
...
@pytest.mark.integration
def test_rpy_search(clear_pkg_cache):
    with make_temp_env("python=3.5") as prefix:
        run_command(
...
def _compile_pyc(use_sys_python=False):
    evs = {}
    with env_vars(evs, stack_callback=conda_tests_ctxt_mgmt_def_pol):
        packages = []
        if use_sys_python:
...
@pytest.mark.integration
def test_compile_pyc_sys_python(clear_pkg_cache):
    return _compile_pyc(use_sys_python=True)


@pytest.mark.integration
def test_compile_pyc_new_python(clear_pkg_cache):
    return _compile_pyc(use_sys_python=False)


@pytest.mark.integration
def test_conda_run_1(clear_pkg_cache):
...
@pytest.mark.integration
def test_conda_run_nonexistant_prefix(clear_pkg_cache):
    with make_temp_env(use_restricted_unicode=False, name=str(uuid4())[:7]) as prefix:
...
@pytest.mark.integration
def test_conda_run_prefix_not_a_conda_env(clear_pkg_cache):
...
@pytest.mark.integration
def test_clone_offline_multichannel_with_untracked(clear_pkg_cache):
    with env_vars(
        {
...
@pytest.mark.integration
def test_package_pinning(clear_pkg_cache):
    with make_temp_env(
...
@pytest.mark.integration
def test_update_all_updates_pip_pkg(clear_pkg_cache):
    with make_temp_env("python=3.6", "pip", "pytz=2018", no_capture=True) as prefix:
        ...
        pip_o, pip_e, _ = run_command(
            Commands.RUN, prefix, "--dev", "python", "-m", "pip", "install", "itsdangerous==0.24"
        )
        PrefixData._cache_.clear()
...
@pytest.mark.integration
def test_package_optional_pinning(clear_pkg_cache):
    with make_temp_env() as prefix:
        run_command(Commands.CONFIG, prefix, "--add", "pinned_packages", "python=3.6.5")
...
@pytest.mark.integration
def test_update_deps_flag_absent(clear_pkg_cache):
...
@pytest.mark.integration
def test_update_deps_flag_present(clear_pkg_cache):
    with make_temp_env("python=2", "itsdangerous=0.24") as prefix:
        assert package_is_installed(prefix, "python=2")
        assert package_is_installed(prefix, "itsdangerous=0.24")
        assert not package_is_installed(prefix, "flask")
...
def test_shortcut_in_underscore_env_shows_message(clear_pkg_cache):
...
@pytest.mark.integration
@pytest.mark.skipif(not on_win, reason="shortcuts only relevant on Windows")
def test_shortcut_not_attempted_with_no_shortcuts_arg(clear_pkg_cache):
    prefix = make_temp_prefix("_" + str(uuid4())[:7])
...
@pytest.mark.integration
@pytest.mark.skipif(not on_win, reason="shortcuts only relevant on Windows")
def test_shortcut_creation_installs_shortcut(clear_pkg_cache):
    shortcut_dir = get_shortcut_dir()
    shortcut_dir = join(
...
@pytest.mark.integration
@pytest.mark.skipif(not on_win, reason="shortcuts only relevant on Windows")
def test_shortcut_absent_does_not_barf_on_uninstall(clear_pkg_cache):
    shortcut_dir = get_shortcut_dir()
    shortcut_dir = join(
        shortcut_dir, "Anaconda{} ({}-bit)" "".format(sys.version_info.major, context.bits)
    )
...
@pytest.mark.integration
@pytest.mark.skipif(not on_win, reason="shortcuts only relevant on Windows")
def test_shortcut_absent_when_condarc_set(clear_pkg_cache):
    shortcut_dir = get_shortcut_dir()
    shortcut_dir = join(
        shortcut_dir, "Anaconda{} ({}-bit)" "".format(sys.version_info.major, context.bits)
    )
...
@pytest.mark.integration
def test_create_default_packages(clear_pkg_cache):
...
@pytest.mark.integration
def test_create_default_packages_no_default_packages(clear_pkg_cache):
    try:
        prefix = make_temp_prefix(str(uuid4())[:7])
...
@pytest.mark.integration
def test_create_dry_run(clear_pkg_cache):
    # Regression test for #3453
    prefix = "/some/place"
    with pytest.raises(DryRunExit):
...
@pytest.mark.integration
def test_create_dry_run_json(clear_pkg_cache):
    prefix = "/some/place"
    with pytest.raises(DryRunExit):
...
@pytest.mark.integration
def test_create_dry_run_yes_safety(clear_pkg_cache):
    with make_temp_env() as prefix:
...
@pytest.mark.integration
def test_packages_not_found(clear_pkg_cache):
    with make_temp_env() as prefix:
        with pytest.raises(PackagesNotFoundError) as exc:
...
@pytest.mark.integration
def test_conda_pip_interop_dependency_satisfied_by_pip(clear_pkg_cache):
    with make_temp_env("python=3.7", "pip", use_restricted_unicode=False) as prefix:
        ...
        output, _, _ = run_command(
            Commands.INSTALL, prefix, "flask", "--dry-run", "--json", use_exception_handler=True
        )
...
def test_conda_pip_interop_pip_clobbers_conda(clear_pkg_cache):
...
@pytest.mark.integration
def test_conda_pip_interop_conda_editable_package(clear_pkg_cache):
...
@pytest.mark.integration
def test_conda_pip_interop_compatible_release_operator(clear_pkg_cache):
    ...
    with make_temp_env(
        ...
        assert int(rc) != 0
        stderr = (
            stderr.decode("utf-8", errors="replace") if hasattr(stderr, "decode") else str(stderr)
        )
...
@pytest.mark.integration
def test_install_freezes_env_by_default(clear_pkg_cache):
...
def test_search_gawk_not_win_filter(clear_pkg_cache):
    with make_temp_env() as prefix:
        stdout, stderr, _ = run_command(
            Commands.SEARCH,
            prefix,
...
def test_search_gawk_on_win(clear_pkg_cache):
    with make_temp_env() as prefix:
...
@pytest.mark.integration
@pytest.mark.skipif(not on_win, reason="gawk is a windows only package")
def test_search_gawk_on_win_filter(clear_pkg_cache):
    with make_temp_env() as prefix:
        stdout, _, _ = run_command(
            Commands.SEARCH,
            prefix,
...
@pytest.mark.integration
def test_bad_anaconda_token_infinite_loop(clear_pkg_cache):
...
@pytest.mark.integration
def test_anaconda_token_with_private_package(clear_pkg_cache):
...
@pytest.mark.integration
def test_use_index_cache(clear_pkg_cache):
...
@pytest.mark.integration
def test_offline_with_empty_index_cache(clear_pkg_cache):
    from conda.core.subdir_data import SubdirData
...
@pytest.mark.integration
def test_create_from_extracted(clear_pkg_cache):
...
@pytest.mark.integration
def test_install_mkdir(clear_pkg_cache):
    try:
...
def test_disallowed_packages(clear_pkg_cache):
    with make_temp_env() as prefix:
        with env_var(
...
@pytest.mark.integration
def test_dont_remove_conda_1(clear_pkg_cache):
...
@pytest.mark.integration
def test_dont_remove_conda_2(clear_pkg_cache):
...
@pytest.mark.integration
def test_force_remove(clear_pkg_cache):
    with make_temp_env() as prefix:
...
@pytest.mark.integration
def test_download_only_flag(clear_pkg_cache):
...
@pytest.mark.integration
def test_transactional_rollback_simple(clear_pkg_cache):
...
@pytest.mark.integration
def test_transactional_rollback_upgrade_downgrade(clear_pkg_cache):
...
@pytest.mark.integration
def test_directory_not_a_conda_environment(clear_pkg_cache):
    prefix = make_temp_prefix(str(uuid4())[:7])
...
@pytest.mark.integration
def test_multiline_run_command(clear_pkg_cache):
    with make_temp_env() as prefix:
...
def test_conda_downgrade(clear_pkg_cache):
...
def test_run_script_called(clear_pkg_cache):
...
def test_post_link_run_in_env(clear_pkg_cache):
...
@pytest.mark.integration
def test_conda_info_python(clear_pkg_cache):
    output, _, _ = run_command(Commands.INFO, None, "python=3.5")
    assert "python 3.5.4" in output


@pytest.mark.integration
def test_toolz_cytoolz_package_cache_regression(clear_pkg_cache):
...
@pytest.mark.integration
def test_remove_spellcheck(clear_pkg_cache):
...
@pytest.mark.integration
def test_conda_list_json(clear_pkg_cache):
    def pkg_info(s):
...
def test_legacy_repodata(clear_pkg_cache):
...
@pytest.mark.integration
@pytest.mark.skipif(context.subdir == "win-32", reason="dependencies not available for win-32")
def test_cross_channel_incompatibility(clear_pkg_cache):
...
def test_neutering_of_historic_specs(clear_pkg_cache):
...
def test_install_bound_virtual_package(clear_pkg_cache):
    with make_temp_env("__glibc>0") as prefix:
        pass


@pytest.mark.integration
def test_remove_empty_env(clear_pkg_cache):
    with make_temp_env() as prefix:
        run_command(Commands.CREATE, prefix)
        run_command(Commands.REMOVE, prefix, "--all")
...
@pytest.mark.integration
def test_remove_ignore_nonenv(clear_pkg_cache):

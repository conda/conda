...
class TestCliInstall(TestCase):
    ...
    @pytest.mark.skipif(
        ...
        reason="Conflict report / analysis is done differently with libmamba.",
    )
        ...
        with patch("conda.resolve.Resolve.find_conflicts") as monkey:
            ...
            with self.assertRaises(UnsatisfiableError):
                stdout, stderr, _ = run_command(
                    Commands.INSTALL, self.prefix, "statistics", "--freeze-installed"
                )
            self.assertEqual(monkey.call_count, 1)
            monkey.reset_mock()
            with self.assertRaises(UnsatisfiableError):
                stdout, stderr, _ = run_command(
                    Commands.CREATE, self.testenv, "statistics", "python=3.7"
                )
            self.assertEqual(monkey.call_count, 1)

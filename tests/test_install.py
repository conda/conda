...
class TestBinaryReplace(unittest.TestCase):
    @pytest.mark.xfail(
        on_win, reason="binary replacement on windows skipped", strict=True
    )
    ...
    def test_multiple(self):
        self.assertEqual(
            binary_replace(b"aaaacaaaa\x00", b"aaaa", b"bbbb"), b"bbbbcbbbb\x00"
        )
        self.assertEqual(
            binary_replace(b"aaaacaaaa\x00", b"aaaa", b"bbb"), b"bbbcbbb\x00\x00\x00"
        )
        self.assertRaises(
            _PaddingError, binary_replace, b"aaaacaaaa\x00", b"aaaa", b"bbbbb"
        )
...
class FileTests(unittest.TestCase):
    ...
    def test_long_default_text(self):
        with open(self.tmpfname, "w") as fo:
            fo.write(
                "#!/opt/anaconda1anaconda2anaconda3/bin/python -O\n" 'echo "Hello"\n'
            )

...
def test_repodata_state_check_mtime(repodata_cache):
    ...
    with pytest.raises(FileNotFoundError):
        cache2.load(state_only=True, binary=False, check_mtime=True)

    with pytest.raises(FileNotFoundError):
        cache2.load(state_only=True, binary=True, check_mtime=True)

    # If (state_only=True, check_mtime=False), load() should not check cache_path_*
...
def test_coverage_conda_http_errors(monkeypatch: MonkeyPatch):
    ...
    with (
        ...
        conda_http_errors("https://conda.anaconda.org", "repodata.json"),
    ):
    ...
    with (
        pytest.raises(UnavailableInvalidChannel),
    ...
    with (
        pytest.raises(CondaHTTPError, match="token"),
        conda_http_errors(
            "/t/dh-73683400-b3ee-4f87-ade8-37de6d395bdb/conda-forge/noarch",
            "repodata.json",
        ),
    ...
    # Oh no
    with (
        pytest.raises(CondaHTTPError, match="A 500-type"),
        conda_http_errors("https://repo.anaconda.com/main/linux-64", "repodata.json"),
    ):
        raise HTTPError(response=Response(500))

    # Ask to unblock URL
    with (
        pytest.raises(CondaHTTPError, match="blocked"),
        conda_http_errors("https://repo.anaconda.com/main/linux-64", "repodata.json"),
    ):
...
def test_ssl_unavailable_error_message():
    try:
        ...
        with (
            pytest.raises(CondaSSLError, match="unavailable"),
            conda_http_errors("https://conda.anaconda.org", "repodata.json"),
        ):

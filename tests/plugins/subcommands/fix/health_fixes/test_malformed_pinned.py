...
def test_empty_pinned_file(
    ...
    """Test when pinned file is empty (treated as no malformed specs)."""
    prefix = tmp_path / "env"
    prefix.mkdir()
    (prefix / "conda-meta").mkdir()
    ...
    mocker.patch(
        "conda.plugins.subcommands.fix.health_fixes.malformed_pinned.find_malformed_pinned_specs",
        return_value=[],
    )

...
def test_solve_1(tmpdir):
    ...
    with get_solver(
        tmpdir, specs_to_add=specs_to_add, prefix_records=final_state, history_specs=specs
    ) as solver:
        final_state = solver.solve_final_state()
        # print(convert_to_dist_str(final_state))
        order = add_subdir_to_iter(
...
@pytest.mark.skipif(
    context.solver_logic in (SolverLogicChoice.LIBMAMBA, SolverLogicChoice.LIBMAMBA_DRAFT),
...
def test_prune_1(tmpdir):
    ...
    with get_solver(
        tmpdir, specs_to_remove=specs_to_remove, prefix_records=final_state_1, history_specs=specs
    ) as solver:
...
def test_update_all_1(tmpdir):
    specs = (
        ...
        MatchSpec(name="system", version="*", build="*0"),
    )
    with get_solver(tmpdir, specs) as solver:
        final_state_1 = solver.solve_final_state()
        # PrefixDag(final_state_1, specs).open_url()
...
def test_auto_update_conda(tmpdir):
    ...
    try:
        ...
        with env_vars(
            {"CONDA_AUTO_UPDATE_CONDA": "no"}, stack_callback=conda_tests_ctxt_mgmt_def_pol
        ):
            specs_to_add = (MatchSpec("pytz"),)
            with get_solver(
                tmpdir, specs_to_add, prefix_records=final_state_1, history_specs=specs
...
def test_aggressive_update_packages(tmpdir):
    ...
    empty_state = ((), ())
    with env_vars(
        {"CONDA_AGGRESSIVE_UPDATE_PACKAGES": ""}, stack_callback=conda_tests_ctxt_mgmt_def_pol
    ):
        base_state = solve(
            empty_state, ["libpng=1.2"], add_subdir_to_iter(("channel-1::libpng-1.2.50-0",))
        )
...
def test_python2_update(tmpdir):
    ...
    with get_solver_4(
        ...
        if context.solver_logic in (SolverLogicChoice.LIBMAMBA, SolverLogicChoice.LIBMAMBA_DRAFT):
...
def test_update_deps_1(tmpdir):
    ...
    with get_solver(
        ...
        final_state_3 = solver.solve_final_state(
            update_modifier=UpdateModifier.UPDATE_DEPS, deps_modifier=DepsModifier.ONLY_DEPS
        )
        pprint(convert_to_dist_str(final_state_3))
        order = add_subdir_to_iter(
            (
...
def test_fast_update_with_update_modifier_not_set(tmpdir):
    ...
    with get_solver_4(
        ...
        if context.solver_logic in (SolverLogicChoice.LIBMAMBA, SolverLogicChoice.LIBMAMBA_DRAFT):
            ...
            assert add_subdir("channel-4::python-3.6.4-hc3d631a_1") in convert_to_dist_str(
                link_precs
            )
        else:
            assert convert_to_dist_str(unlink_precs) == unlink_order
...
def test_remove_with_constrained_dependencies(tmpdir):
    ...
    with get_solver_4(
        tmpdir, specs_to_remove=specs_to_remove, prefix_records=link_dists_1, history_specs=specs
    ) as solver:
...
def test_priority_1(tmpdir):
    ...
    with env_var("CONDA_SUBDIR", "linux-64", stack_callback=conda_tests_ctxt_mgmt_def_pol):
        ...
        with env_var(
            "CONDA_CHANNEL_PRIORITY", priority, stack_callback=conda_tests_ctxt_mgmt_def_pol
        ):
            with get_solver_aggregate_1(tmpdir, specs) as solver:
                final_state_1 = solver.solve_final_state()
                pprint(convert_to_dist_str(final_state_1))
        ...
        with env_var(
            "CONDA_CHANNEL_PRIORITY", "disabled", stack_callback=conda_tests_ctxt_mgmt_def_pol
        ):
            with get_solver_aggregate_1(
...
def test_features_solve_1(tmpdir):
    ...
    with env_var(
        "CONDA_CHANNEL_PRIORITY", "flexible", stack_callback=conda_tests_ctxt_mgmt_def_pol
    ):
        with get_solver_aggregate_1(tmpdir, specs) as solver:
            final_state_1 = solver.solve_final_state()
            pprint(convert_to_dist_str(final_state_1))
...
def test_current_repodata_usage(tmpdir, solver_logic):
...
def test_current_repodata_fallback(tmpdir, solver_logic):
    solver = solver_logic(
        tmpdir.strpath,
        (Channel(CHANNEL_DIR),),
        ("win-64",),
...
def test_downgrade_python_prevented_with_sane_message(tmpdir):
    ...
    with get_solver(
        ...
        error_msg = str(exc.value).strip()
        if context.solver_logic == SolverLogicChoice.CLASSIC:
            assert (
            ...
            raise ValueError(
                f"Unrecognized solver logic which cannot be tested: {context.solver_logic}"
            )
    ...
    with get_solver(
        ...
        if context.solver_logic == SolverLogicChoice.CLASSIC:
            ...
            warnings.warn(
                "!!! This test passed with a message that is not as informative as Conda's. "
...
def test_packages_in_solution_change_already_newest(tmpdir, solver_logic):
    specs = MatchSpec("mypkg")
    pre_packages = {"mypkg": [("mypkg", "0.1.1")]}
    post_packages = {"mypkg": [("mypkg", "0.1.1")]}
    solver = solver_logic(tmpdir, (Channel(CHANNEL_DIR),), ("linux-64",), specs_to_add=[specs])
    constrained = solver.get_constrained_packages(pre_packages, post_packages, fake_index)
    assert len(constrained) == 0


def test_packages_in_solution_change_needs_update(tmpdir, solver_logic):
    specs = MatchSpec("mypkg")
...
def test_packages_in_solution_change_constrained(tmpdir, solver_logic):
    specs = MatchSpec("mypkg")
    pre_packages = {"mypkg": [("mypkg", "0.1.0")]}
...
def test_determine_constricting_specs_conflicts(tmpdir, solver_logic):
    solution_prec = [
        PrefixRecord(
            package_type=PackageType.NOARCH_GENERIC,
            name="mypkg",
    ...
    solver = solver_logic(tmpdir, (Channel(CHANNEL_DIR),), ("linux-64",), specs_to_add=[spec])
    constricting = solver.determine_constricting_specs(spec, solution_prec)
    assert any(i for i in constricting if i[0] == "mypkgnot")


def test_determine_constricting_specs_conflicts_upperbound(tmpdir, solver_logic):
    solution_prec = [
        PrefixRecord(
            package_type=PackageType.NOARCH_GENERIC,
            name="mypkg",
...
def test_determine_constricting_specs_multi_conflicts(tmpdir, solver_logic):
    solution_prec = [
        PrefixRecord(
            package_type=PackageType.NOARCH_GENERIC,
            name="mypkg",
...
def test_determine_constricting_specs_no_conflicts_version_star(tmpdir, solver_logic):
    solution_prec = [
        PrefixRecord(
            package_type=PackageType.NOARCH_GENERIC,
            name="mypkg",
...
def test_determine_constricting_specs_no_conflicts_free(tmpdir, solver_logic):
    solution_prec = [
        PrefixRecord(
            package_type=PackageType.NOARCH_GENERIC,
            name="mypkg",
...
def test_determine_constricting_specs_no_conflicts_no_upperbound(tmpdir, solver_logic):
    solution_prec = [
        PrefixRecord(
            package_type=PackageType.NOARCH_GENERIC,
            name="mypkg",

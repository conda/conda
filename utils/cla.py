...
contributors = {line.strip() for line in output}
...
with open(".cla-signers") as fh:
    ...
    cla_signers = {line.split("|")[1].strip() for line in cla_signer_lines}
...
with open(".exempt-commits") as fh:
    ...
    exempt_commits = {line.split("|")[0].strip() for line in exempt_commits_lines}

with open('.github-map') as fh:
    github_map = {
        git_id.strip(): ghusername.strip()
        for ghusername, git_id in (
...
contributions = tuple(
    (git_id, commits)
    for commits, git_id in (line.strip().split(" ", 1) for line in output)
    if git_id in missing_signatures
)
contributions_map = {
    git_id: int(commits)
    for commits, git_id in (line.strip().split(" ", 1) for line in output)
...
for git_id in missing_signatures:
    output = check_output(
        f'git log --author="{git_id}" --use-mailmap --format="%H" --shortstat', shell=True
    )
...
for git_id in sorted(missing_signatures_stats, key=lambda x: (missing_signatures_stats[x][0], missing_signatures_stats[x][1]), reverse=True):
    ...
    output = check_output("git show {}".format(" ".join(stats[3])), shell=True)

...
install_conda_shell_scripts() {
    ...
    if [ -n "$ON_WIN" ]; then
        ...
        echo "_CONDA_ROOT=\"$win_prefix\"" >> "$prefix/$BIN_DIR/activate"
        ...
        echo "_CONDA_ROOT=\"$win_prefix\"" >> "$prefix/$BIN_DIR/deactivate"
        ...
        echo "_CONDA_EXE=\"$win_conda_exe\"" > "$prefix/etc/profile.d/conda.sh"
        echo "_CONDA_ROOT=\"$win_prefix\"" >> "$prefix/etc/profile.d/conda.sh"

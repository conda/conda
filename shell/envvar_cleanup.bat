...
@IF /I NOT "%VARIABLE%"=="" (
    ...
    @IF /I "%MODE%"=="duplicate" (
        ...
        @FOR /L %%i IN (1,1,!MAX_ITER!) DO @(
            ...
            @FOR /F "delims=" %%j IN ("!x!") DO @SET "tmp_VARIABLE=!VARIABLE:%DELIM%%%j%DELIM%=%DELIM%!"
            ...
            @IF /I "!tmp_VARIABLE!"=="!VARIABLE!" @SET "VARIABLE=!VARIABLE!!x!%DELIM%"
        )
    ) ELSE (
        @REM iterate over all phrases split by delim
            ...
            @FOR /L %%j IN (0,1,!STR_TO_REMOVE_I!) DO @(
                @IF /I NOT "!STR_TO_REMOVE[%%j]!"=="" (
                    ...
                    @IF /I "!STR_TO_REMOVE[%%j]!"=="!x!" (
                        ...
                        @IF /I "%FUZZY%"=="%TRUE%" (
                            @FOR /F "delims=" %%k IN ("!STR_TO_REMOVE[%%j]!") DO @SET "tmp_VARIABLE=!x:%%k=!"
                            @IF /I NOT "!tmp_VARIABLE!"=="!x!" (
                                @SET "FUZZY_MATCH=%%j"
                            )
                        )
                    )
